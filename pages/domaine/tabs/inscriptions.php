        ...
        <div class="tab-content">
            <?php if ($inscription_sub_tab === 'liste'): ?>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="btn-group">
                        <button type="button" class="btn btn-outline-secondary btn-sm" id="selectAllInscriptionsBtn" onclick="toggleSelectAllInscriptions()">
                            <i class="bi bi-check2-square"></i> Tout sélectionner
                        </button>
                        <button type="button" class="btn btn-danger btn-sm" id="btnDeleteSelected" onclick="submitBulkDelete()" disabled>
                            <i class="bi bi-trash"></i> Supprimer la sélection (<span id="bulkDeleteCount">0</span>)
                        </button>
                ...
                <form method="POST"
                    action="?page=domaine&action=view&id=<?php echo $id_domaine; ?>&mention=<?php echo $mention_id; ?>&tab=inscriptions&promotion=<?php echo $promotion_code; ?>&annee=<?php echo $annee_academique; ?>&sub_tab=liste"
                    id="bulkDeleteForm">
                    <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
                    <input type="hidden" name="action_inscription" value="desinscrire_multiple">
                                ...
                                <thead class="table-dark">
                                    <tr>
                                        <th width="4%" class="text-center">
                                            <input type="checkbox" class="form-check-input" id="selectAllInscriptionsHeader" onclick="toggleSelectAllFromHeader(this)">
                                        </th>
                                        ...
                                        <tr class="inscription-row">
                                            <td class="text-center">
                                                <input type="checkbox"
                                                    ...
                                                    name="selected_inscriptions[]"
                                                    value="<?php echo (int) $inscription['id_inscription']; ?>"
                                            ...
                                            <td class="text-center">
                                                <span class="badge bg-<?php echo $inscription['badge_class']; ?> px-3 py-2">
                                                    <i class="bi bi-<?php
                                                                    echo $inscription['statut'] === 'Actif' ? 'check-circle' : ($inscription['statut'] === 'En attente' ? 'clock' : 'x-circle');
                                                                    ?>"></i>
            ...
            <?php elseif ($inscription_sub_tab === 'reinscrire'): ?>
                        ...
                        <div class="card border-0 shadow-sm mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <form method="POST"
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-4">
                                            <label for="operation_mode" class="form-label fw-semibold">
                                                <i class="bi bi-sliders me-1"></i>
                                                Opération
                                            </label>
                                            <select class="form-select" id="operation_mode">
                                                <option value="reinscrire" selected>Réinscrire</option>
                                                <option value="deplacer">Déplacer</option>
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            <label for="source_mention_unique" class="form-label fw-semibold">
                                                <i class="bi bi-funnel me-1"></i>
                                                Mention source
                                            </label>
                                            <select class="form-select" id="source_mention_unique">
                                                <option value="">Toutes les mentions</option>
                                                <?php
                                                try {
                                                    $stmt_mentions_source = $pdo->prepare(" 
                                                        ...
                                                        ORDER BY m.libelle
                                                    ");
                                                    $stmt_mentions_source->execute([$id_domaine]);
                                                    while ($mention_source = $stmt_mentions_source->fetch(PDO::FETCH_ASSOC)):
                                                ?>
                                                        <option value="<?php echo (int)$mention_source['id_mention']; ?>">
                                                            <?php echo htmlspecialchars($mention_source['libelle'] . ' (' . $mention_source['code_mention'] . ')'); ?>
                                                        </option>
                                                <?php
                                                    endwhile;
                                                } catch (PDOException $e) {
                                        ...
                                        <div class="col-md-4">
                                            <label for="source_promotion_unique" class="form-label fw-semibold">
                                                <i class="bi bi-funnel-fill me-1"></i>
                                                Promotion source
                                            </label>
                                            <select class="form-select" id="source_promotion_unique">
                                                <option value="">Toutes les promotions</option>
                                                <?php
                                                try {
                                                    $stmt_promotions_source = $pdo->prepare("SELECT code_promotion, nom_promotion FROM t_promotion ORDER BY nom_promotion");
                                                    $stmt_promotions_source->execute();
                                                    while ($promotion_source = $stmt_promotions_source->fetch(PDO::FETCH_ASSOC)):
                                                ?>
                                                        <option value="<?php echo htmlspecialchars($promotion_source['code_promotion']); ?>">
                                                            <?php echo htmlspecialchars($promotion_source['nom_promotion'] . ' (' . $promotion_source['code_promotion'] . ')'); ?>
                                                        </option>
                                                <?php
                                                    endwhile;
                                                } catch (PDOException $e) {
                                                ...
                                                <table class="table table-hover mb-0">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        try {
                                                            ...
                                                            $annee_id_courante = (int)$annee_academique;

                                                            // Liste unique des étudiants inscrits (actifs) du domaine,
                                                            // puis filtrage fin côté interface (source + mode).
                                                            $query_etudiants = "
                                                                SELECT DISTINCT
                                                                    e.matricule,
                                                                    e.nom_etu,
                                                            ...
                                                            $stmt_etudiants = $pdo->prepare($query_etudiants);
                                                                                                                        $stmt_etudiants->execute([$id_domaine]);
                                                        ...
                                                        ?>
                                                                    <tr class="etudiant-row"
                                                                        data-mention-id="<?php echo (int)$etudiant['id_mention']; ?>"
                                                                        data-promo-code="<?php echo htmlspecialchars($etudiant['code_promotion']); ?>"
                                                                        data-is-current-year="<?php echo ((int)$etudiant['id_annee'] === $annee_id_courante) ? '1' : '0'; ?>">
                                                                        <td>
                                                                            <div class="form-check">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-4">
                                        ...
                                        <button type="submit" class="btn btn-primary btn-lg" id="btnReinscrire" disabled>
                                            ...
                                            <span id="operation-label">Réinscrire</span> <span id="nombre-selection">0</span> étudiant(s)
                                        </button>
                                    </div>
                                </form>
                            </div>
                ...
                <script>
                    // Tableau unique: réinscrire et déplacer
                    document.addEventListener('DOMContentLoaded', function() {
                        const operationMode = document.getElementById('operation_mode');
                        const actionInput = document.querySelector('#formReinscrire input[name="action_inscription"]');
                        const sourceMention = document.getElementById('source_mention_unique');
                        const sourcePromotion = document.getElementById('source_promotion_unique');
                        ...
                        function applyFilters() {
                            const selectedMention = sourceMention ? sourceMention.value : '';
                            const selectedPromotion = sourcePromotion ? sourcePromotion.value : '';
                            const mode = operationMode ? operationMode.value : 'reinscrire';

                            rows.forEach(row => {
                                const mentionOk = !selectedMention || row.dataset.mentionId === selectedMention;
                                const promoOk = !selectedPromotion || row.dataset.promoCode === selectedPromotion;
                                const yearOk = mode === 'deplacer' ? row.dataset.isCurrentYear === '1' : true;
                                const visible = mentionOk && promoOk && yearOk;

                                row.style.display = visible ? '' : 'none';
                                if (!visible) {
                                    const cb = row.querySelector('.etudiant-checkbox');
                        ...
                        function updateCounter() {
                            const checked = document.querySelectorAll('.etudiant-row:not([style*="display: none"]) .etudiant-checkbox:checked').length;
                        ...
                        function updateModeUI() {
                            const mode = operationMode ? operationMode.value : 'reinscrire';
                            const isMove = mode === 'deplacer';

                            actionInput.value = isMove ? 'deplacer' : 'reinscrire';
                            operationLabel.textContent = isMove ? 'Déplacer' : 'Réinscrire';
                            btnReinscrire.classList.toggle('btn-warning', isMove);
                            btnReinscrire.classList.toggle('btn-primary', !isMove);
                        }
                        ...
                        if (sourceMention) sourceMention.addEventListener('change', applyFilters);
                        if (sourcePromotion) sourcePromotion.addEventListener('change', applyFilters);
                        if (operationMode) operationMode.addEventListener('change', function() {
                            updateModeUI();
                            applyFilters();
                        });
                        ...
                        form.addEventListener('submit', function(e) {
                            ...
                            const actionLabel = mode === 'deplacer' ? 'déplacer' : 'réinscrire';
                            if (!confirm(`Êtes-vous sûr de vouloir ${actionLabel} ${checked} étudiant(s) ?`)) {
                                e.preventDefault();
                                return false;
                            }
                        });
                ...
                <script>
                    ...
                    function getSelectedInscriptionCount() {
                        return document.querySelectorAll('.inscription-checkbox:checked').length;
                    }

                    function updateBulkDeleteState() {
                        const selected = getSelectedInscriptionCount();
                        const btn = document.getElementById('btnDeleteSelected');
                        const count = document.getElementById('bulkDeleteCount');
                        const header = document.getElementById('selectAllInscriptionsHeader');
                    ...
                    function toggleSelectAllFromHeader(source) {
                        const checkboxes = document.querySelectorAll('.inscription-checkbox');
                        checkboxes.forEach(cb => cb.checked = source.checked);
                        updateBulkDeleteState();
                    }

                    function toggleSelectAllInscriptions() {
                        const checkboxes = Array.from(document.querySelectorAll('.inscription-checkbox'));
                        const selected = checkboxes.filter(cb => cb.checked).length;
                        const shouldSelectAll = selected !== checkboxes.length;
                    ...
                    function submitBulkDelete() {
                        ...
                        const message = `Supprimer ${selected} inscription(s) sélectionnée(s) ? Cette action est irréversible.`;
                        if (!confirm(message)) {
                            return;
                        }
                    ...
                    function confirmerDesinscription(idInscription, nomEtudiant) {
                        const submitSingleDelete = () => {
                            ...
                            action.type = 'hidden';
                            action.name = 'action_inscription';
                            action.value = 'desinscrire';
                            form.appendChild(action);

                            const id = document.createElement('input');
                            id.type = 'hidden';
                            id.name = 'id_inscription';
                            id.value = String(idInscription);
                            form.appendChild(id);
                    ...
                    function exportToCSV() {
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            for (let j = 1; j < cols.length - 1; j++) { // Exclure la colonne checkbox + Actions
